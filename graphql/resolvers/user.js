...
export default {
     Query: {
         user: async (root, args) => {
          const user = await User.findById(args._id).populate("movies");
          return user;
         },
         users: async () => {
    ...
    Mutation: {
        createUser: async(root, args) => {
             ...
             if(!validator.isLength(userData.password, {min: 4, max: 255})){
                throw new UserInputError (`Password has to be between 4 and 255 symbols`, {
                    field: "password",
        ...
        login: async(root, {email, password}) => {
            ...
            if (!validPassword){
                throw new UserInputError (`Password is incorrect ${password}`, {
                    field: "password",
                    value: password,
